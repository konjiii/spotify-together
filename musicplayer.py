...
class MusicPlayer:
    ...
    #         query = input(f"Add: {tracks[0]["name"]}, to queue?\nthe Enter key: yes\ntype a number: the search result number\n'n': search again")
    #         match query:
    #             case int(query):
    #                 if query >= len(tracks) or query < 0:
    #                     pass
    #                 else:
    #                     chosen_track = tracks[query]
    ...
    def loop(self):
        ...
        while not self.exit.is_set():
            ...
            if self.index >= self.get_playlist_length(self.playlist_uri):
                ...
                self.index = self.get_playlist_length(self.playlist_uri)+self.index
